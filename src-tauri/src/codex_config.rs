...
"#;
    ...
    fn resolve_catalog_path_returns_none_when_config_missing_field() {
        let generated = PathBuf::from("/tmp/.codex/ai-coding-model-catalog.json");
...
"#;
        let generated_path = PathBuf::from("/home/user/.codex/ai-coding-model-catalog.json");
...
"#;
        ...
        assert_eq!(
            result, None,
            "user-owned catalog should not be claimed by ai-coding"
        );
    }

    #[test]
    fn set_catalog_json_none_removes_relative_path() {
        let input = r#"model_catalog_json = "ai-coding-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove relative ai-coding-owned field"
        );
    }
}
