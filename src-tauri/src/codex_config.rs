...
pub const CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "ai-coding-model-catalog.json";
...
const CODEX_WEB_SEARCH_REJECT_HOSTS: &[&str] = &[
    ...
    "bigmodel.cn",    // Zhipu GLM CN
    "z.ai",           // Zhipu GLM global
];
...
/// ai-coding–maintained catalog file referenced by `~/.codex/config.toml` and
...
/// ai-coding–generated file (identified by filename
/// `ai-coding-model-catalog.json`). A user-managed external catalog file is
...
/// Read the ai-coding Codex model catalog file with a size cap.
...
/// Given `config.toml` text, resolve the on-disk path of the ai-coding–owned
...
pub(crate) fn resolve_cc_switch_catalog_path(
    ...
    // /etc）能让 `link/ai-coding-model-catalog.json` 通过上面的检查，读取却
...
/// Flip a proxy-managed OAuth card's active custom provider from the legacy
/// `requires_openai_auth = true` shape to its safe keyless representation.
/// The local proxy injects these providers' credentials; leaving this true can
/// make Codex fall back to the preserved official ChatGPT login.
...
mod tests {
    ...
    fn seed_rotated_managed_codex_live_state() -> CodexLiveTestState {
        ...
        crate::config::write_text_file(
            ...
            "# cas-guard-sentinel\nmodel = \"gpt-5.5\"\nmodel_catalog_json = \"ai-coding-model-catalog.json\"\n",
        )
...
"#;
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        let with_catalog = "model_catalog_json = \"ai-coding-model-catalog.json\"\n";
...
"#;
    ...
    fn neutralize_proxy_oauth_fallback_flips_only_active_custom_true() {
        let unsafe_config = "model_provider = \"custom\"\nmodel = \"grok-4.5\"\n\n[model_providers.custom]\nname = \"xai\"\nbase_url = \"https://api.x.ai/v1\"\nwire_api = \"responses\"\nrequires_openai_auth = true\n";
        let neutralized = neutralize_codex_official_auth_fallback_for_proxy_oauth(unsafe_config)
            .expect("active custom flag should be neutralized");
        ...
        assert!(
            neutralize_codex_official_auth_fallback_for_proxy_oauth(inline)
                .expect("inline provider table should be supported")
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/ai-coding-model-catalog.json");
...
"#;
    ...
    fn resolve_catalog_path_accepts_cc_switch_owned_file() {
        let base = PathBuf::from("/tmp/.codex");
        let config = r#"model_catalog_json = "/tmp/.codex/ai-coding-model-catalog.json"
"#;
...
model = "glm-5"
"#;
        // Simulate a WSL UNC path as ai-coding would see it on Windows;
        // the function now writes just the relative filename.
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\ai-coding-model-catalog.json");
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/ai-coding-model-catalog.json");
    ...
    fn set_catalog_json_none_removes_cc_switch_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/ai-coding-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove ai-coding-owned field regardless of path format"
        );
    }
...
"#;
    ...
    fn resolve_catalog_rejects_absolute_path_outside_config_dir() {
        let config_text = r#"model_catalog_json = "/tmp/secret/ai-coding-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_accepts_absolute_path_inside_config_dir() {
        let config_text = r#"model_catalog_json = "/home/user/.codex/ai-coding-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_rejects_traversal_to_parent_directory() {
        let config_text = r#"model_catalog_json = "../ai-coding-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_rejects_symlink_escaping_config_dir() {
        ...
        // "link/ai-coding-model-catalog.json" 词法上在 base 内，真实读取却落到
        ...
        let config_text = r#"model_catalog_json = "link/ai-coding-model-catalog.json"
"#;
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
            result, None,
