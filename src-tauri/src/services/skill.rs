...
pub enum SkillStorageLocation {
    /// AI Coding 管理目录 (~/.ai-coding/skills/)
    #[default]
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.ai-coding/skills/ 或 ~/.agents/skills/）
    pub fn get_ssot_dir() -> Result<PathBuf> {
        let location = crate::settings::get_skill_storage_location();
        let dir = match location {
            SkillStorageLocation::AiCoding => get_app_config_dir().join("skills"),
    ...
    /// 获取 Skill 卸载备份目录（~/.ai-coding/skill-backups/）
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 AI Coding 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "ai-coding".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 AI Coding 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "ai-coding".to_string()));
    ...
    fn resolve_uninstall_backup_source_excluding(
        ...
        // 返回值会被整目录复制进 ~/.zhongguoai/skill-backups/ 并由 get_skill_backups
...
mod tests {
    ...
    fn uninstall_warns_when_the_pi_root_cannot_be_resolved() {
        let _location = StorageLocationGuard::set(SkillStorageLocation::AiCoding);
        let temp = tempdir().expect("tempdir");
        let _home = TestHomeGuard::set(temp.path());
        let _pi_dir =
    ...
    fn restore_from_backup_rejects_traversal_directory_in_metadata() {
        ...
        // SSOT 位于 {home}/.zhongguoai/skills，"../../pwned-restore" 若生效会写到 {home}/pwned-restore。
    ...
    fn migrate_storage_safely_leaves_an_existing_pi_ssot_alias() {
        ...
        let result = SkillService::migrate_storage(&db, SkillStorageLocation::AiCoding)
    ...
    fn uninstall_rejects_traversal_directory_from_db_row() {
        ...
        // 与 import_sql_string_for_sync 的效果一致）。SSOT = {home}/.zhongguoai/skills，
    ...
    fn migrate_storage_retargets_an_equivalent_relative_pi_symlink() {
        ...
        std::os::unix::fs::symlink(Path::new("../../.zhongguoai/skills/test-skill"), &pi_skill)
