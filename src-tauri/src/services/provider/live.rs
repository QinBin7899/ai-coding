...
/// Proxy-managed OAuth cards are keyless because the local proxy injects their
/// live token. Neutralize legacy snapshots that would otherwise make Codex read
/// the preserved official `auth.json` for a third-party endpoint.
...
fn restore_live_settings_for_provider_backfill(
    ...
    // `modelCatalog` is a ai-coding–private field whose SSOT is the DB. Live's
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        AppType::Codex => {
            ...
            // `modelCatalog` is a ai-coding private field that lives only in
            // the DB SSOT plus the `ai-coding-model-catalog.json` projection
...
/// into the AI Coding database. Each provider found will be added to the
