...
struct ParsedCodexFile {
    ...
    /// Stable logical thread ID from the root `session_meta` record.
...
/// Returns the first UUID from a revert/resume filename ending in
/// `<thread_id>_<rollout_id>.jsonl`. Single-UUID filenames return `None`.
...
fn sync_single_codex_file(
    ...
    // The filename's trailing rollout UUID remains part of request_id, while
    // session_id uses the stable root metadata identity for resumed rollouts.
...
mod tests {
    ...
    #[test]
    fn test_resumed_rollout_accepts_leading_metadata_uuid() -> Result<(), AppError> {
        ...
        assert_eq!(parsed.meta_thread_id.as_deref(), Some(PARENT_ID));
        assert!(parsed.has_billable_tokens);
        Ok(())
    }

    #[test]
    fn test_resumed_rollout_persists_metadata_session_id() -> Result<(), AppError> {
    ...
    #[test]
    fn test_single_uuid_filename_metadata_mismatch_stays_deferred() -> Result<(), AppError> {
        let dir = tempdir().unwrap();
