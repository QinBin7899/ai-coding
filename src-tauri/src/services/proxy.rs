...
///
/// 原因：接管模式下 `*_MODEL` 必须由 AI Coding 写成稳定的 Claude 角色别名，
...
pub struct ProxyService {
    ...
    /// 最近一次 `get_status()` / `start()` 探测到的「外部代理实例正在运行」结果。
    /// `is_running()` 只读这个缓存，不再自己发 HTTP 探测——它会被
    /// `futures::executor::block_on` 在同步上下文（live 同步、托盘菜单）里调用，
    /// 在 current_thread runtime 上发网络请求会直接死锁。
    external_proxy_detected: Arc<std::sync::atomic::AtomicBool>,
}
...
impl ProxyService {
    ...
    pub fn new_with_codex_oauth_manager(
        ...
        Self {
            ...
            external_proxy_detected: Arc::new(std::sync::atomic::AtomicBool::new(false)),
        }
    }
    ...
    fn probe_base_url_for_config(config: &ProxyConfig) -> String {
        let host = match config.listen_address.trim() {
    ...
    /// 探测是否已有另一个实例（例如同机的另一个 AI Coding 进程）在配置端口上
    /// 提供代理服务：先试 `/status`，失败再试 `/health`。
    async fn probe_external_proxy_status(config: &ProxyConfig) -> Option<ProxyStatus> {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .ok()?;
        let base_url = Self::probe_base_url_for_config(config);

        let status_url = format!("{base_url}/status");
        if let Ok(response) = client.get(&status_url).send().await {
            if response.status().is_success() {
                if let Ok(mut status) = response.json::<ProxyStatus>().await {
                    status.running = true;
                    if status.address.trim().is_empty() {
                        status.address = config.listen_address.clone();
                    }
                    if status.port == 0 {
                        status.port = config.listen_port;
                    }
                    return Some(status);
                }
            }
    ...
    pub async fn start(&self) -> Result<ProxyServerInfo, String> {
        ...
        if let Some(status) = Self::probe_external_proxy_status(&config).await {
            log::warn!(
                "检测到已有外部代理实例在运行，复用现有监听: {}:{}",
                status.address,
                status.port
            );
            self.external_proxy_detected
    ...
    pub async fn get_status(&self) -> Result<ProxyStatus, String> {
        if let Some(server) = self.server.read().await.as_ref() {
            ...
            let config = self
                ...
                .await
                .map_err(|e| format!("获取代理配置失败: {e}"))?;
            let external = Self::probe_external_proxy_status(&config).await;
            self.external_proxy_detected
                .store(external.is_some(), std::sync::atomic::Ordering::Relaxed);
            if let Some(status) = external {
                return Ok(status);
            }
    ...
    /// 检查服务器是否正在运行
    ///
    /// 本实例没起服务器时，沿用最近一次 `get_status()` / `start()` 的外部实例探测结果
    /// （UI 会周期性轮询状态，所以缓存最多滞后一个轮询周期）。这里刻意不做网络探测，
    /// 因为调用方可能在 `futures::executor::block_on` 里同步等待。
    ...
    /// Synchronous, non-probing check for startup UI paths such as the tray menu.
...
"#;
        ...
        assert!(
            catalog_path.exists(),
            "ai-coding-model-catalog.json must be created on provider switch"
        );
...
"#;
        ...
        assert!(
            message.contains("写入 Codex 配置失败")
                ...
                || (message.contains("捕获 Codex 热切换前状态失败")
                    && message
                        .contains(crate::codex_config::CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME)),
    ...
    async fn codex_restore_from_backup_preserves_model_catalog_pointer() {
        ...
        // Pre-takeover Live state: config.toml points at the ai-coding generated
        ...
        assert!(
            ...
            "restored pointer must still reference the ai-coding generated catalog file"
        );
    }
    ...
    async fn codex_restore_from_backup_projects_inline_model_catalog() {
        ...
        assert!(
            catalog_path.exists(),
            "restore must generate the ai-coding catalog file on disk"
        );
    ...
    async fn codex_restore_empty_auth_backup_still_projects_inline_catalog() {
        ...
        assert!(
            ...
            "empty-auth restore must generate the ai-coding catalog file"
        );
        assert!(
            !crate::codex_config::get_codex_auth_path().exists(),
