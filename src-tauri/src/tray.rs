...
pub const TRAY_ID: &str = "ai-coding";
...
pub fn create_tray_menu(
    ...
    let is_proxy_running = app_state.proxy_service.is_running_sync();
    ...
    for section in TRAY_SECTIONS.iter() {
        ...
        if providers.is_empty() {
            ...
            let is_app_taken_over = is_proxy_running
                && (app_state.db.has_live_backup_sync(app_type_str)
                    || app_state
                        .proxy_service
...
pub fn handle_tray_menu_event(app: &tauri::AppHandle, event_id: &str) {
    ...
    match event_id {
        ...
        "open_website" => {
            if let Err(e) = app.opener().open_url("https://aicoding.io", None::<String>) {
