...
/// catalog with a top-level `models` field.  Return the ai-coding–managed model
...
/// ai-coding–owned `model_catalog_json`, using the same path ownership rules as
...
pub async fn handle_models() -> Result<Json<Value>, ProxyError> {
        ...
        active_catalog_path.as_ref().filter(|path| path.exists())
    {
        ...
        if active_catalog_path.is_none() {
            log::debug!(
                "[models] stale guard: catalog not served (model_catalog_json not set to ai-coding catalog)"
            );
        }
        json!({"models": []})
    };
...
async fn handle_responses_for_app(
    ...
    // Native Responses passthrough to xAI: restore flattened function names and
    // normalize completed tool arguments. This must run even when the request
    // had no namespace tools, because Grok may emit whole-number JSON floats.
...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        ...
        // 避免「以为是 AI Coding 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not a AI Coding limit. ",
        ...
        let status_fragment = upstream_status
            ...
            .unwrap_or_default();
        format!(
            "AI Coding local proxy failed while handling Codex endpoint {endpoint}. Provider: {provider_name}; model: {request_model}{status_fragment}; cause: {cause}"
        )
    };
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert!(message.contains("AI Coding local proxy failed"));
