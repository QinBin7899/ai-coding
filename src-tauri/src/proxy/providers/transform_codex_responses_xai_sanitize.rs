...
mod tests {
    ...
    #[test]
    fn whole_floats_are_normalized_without_crossing_u64_boundary() {
        let mut value: Value =
        ...
        let mut boundary: Value =
            serde_json::from_str(r#"{"big":18446744073709551616.0}"#).unwrap();
        assert!(!rewrite_whole_number_floats(&mut boundary));
        assert_eq!(boundary["big"].as_f64(), Some(18_446_744_073_709_551_616.0));

        let mut below: Value = serde_json::from_str(r#"{"big":18446744073709549568.0}"#).unwrap();
        assert!(rewrite_whole_number_floats(&mut below));
        assert_eq!(below["big"].as_u64(), Some(18_446_744_073_709_549_568));
    }

    #[test]
    fn completed_arguments_are_rewritten_but_deltas_are_untouched() {
        let mut done = json!({
            "type": "response.function_call_arguments.done",
            "arguments": r#"{"session_id":92116.0,"nested":{"n":120000.0},"wait":1.5}"#
        });
        assert!(normalize_xai_function_call_integer_arguments(&mut done));
        ...
        assert_eq!(arguments["nested"]["n"].as_u64(), Some(120000));
        assert_eq!(arguments["wait"].as_f64(), Some(1.5));

        let mut delta = json!({
            "type": "response.function_call_arguments.delta",
            "delta": r#"{"session_id":92116.0"#,
            "item": {"type": "function_call", "arguments": r#"{"session_id":92116.0}"#}
        });
        let original = delta.clone();
        assert!(!normalize_xai_function_call_integer_arguments(&mut delta));
        assert_eq!(delta, original);
    }

    #[test]
    fn rewrites_agent_message_and_unknown_model_together() {
        let mut body = json!({
            "model": "gpt-5.6-sol",
            "input": [{
                "type": "agent_message",
        ...
        assert_eq!(body["input"][0]["id"], "amsg_child");
        assert_eq!(body["input"][0]["content"][1]["text"], "Review the diff.");
        assert!(body["input"][0].get("author").is_none());
    }

    #[test]
    fn preserves_grok_models_but_remaps_alien_subagent_models() {
        let allowed = collect_xai_catalog_model_ids(&json!({
            "modelCatalog": {"models": [{"slug": "grok-4.6"}]}
        }));

        let mut grok = json!({"model": "xai/Grok-4.7-Fast"});
        assert_eq!(
            rewrite_xai_unknown_request_model(&mut grok, "grok-4.6", &allowed),
            None
        );

        let mut alien = json!({"model": "gpt-5.6-sol"});
        assert_eq!(
            rewrite_xai_unknown_request_model(&mut alien, "grok-4.6", &allowed),
            Some(("gpt-5.6-sol".to_string(), "grok-4.6".to_string()))
        );
        assert_eq!(alien["model"], "grok-4.6");

        let mut non_ascii = json!({"model": "模型"});
        assert!(rewrite_xai_unknown_request_model(&mut non_ascii, "grok-4.6", &allowed).is_some());
    }
