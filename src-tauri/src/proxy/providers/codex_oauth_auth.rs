...
/// User-Agent
const CODEX_USER_AGENT: &str = "ai-coding-codex-oauth";
...
mod tests {
    ...
    async fn concurrent_duplicate_workspace_and_user_identity_is_rejected() {
        ...
        drop(manager);
        assert_eq!(CodexOAuthManager::new(path).list_accounts().await.len(), 1);
    }
    ...
    async fn duplicate_legacy_workspace_and_user_identity_is_rejected() {
        ...
        let legacy = serde_json::json!({
            ...
            "accounts": {
                "shared-workspace": {
                    ...
                    "id_token": crate::codex_config::test_codex_id_token("same-user"),
                    "authenticated_at": 1
                }
            },
        ...
        let result = manager
            ...
            .await;
        assert!(matches!(result, Err(CodexOAuthError::DuplicateAccount)));
        assert_eq!(manager.list_accounts().await.len(), 1);
        assert!(manager.refresh_locks.read().await.is_empty());
    }

    #[tokio::test]
    async fn targeted_reauth_rechecks_duplicates_outside_target() {
        let temp = tempfile::tempdir().unwrap();
        let manager = CodexOAuthManager::new(temp.path().to_path_buf());
        let target = manager
            .add_account_internal(
