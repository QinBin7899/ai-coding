...
/// tool declarations flattened before forwarding, plus xAI request/response
/// compatibility rewrites.
///
/// Codex 0.142+ emits ChatGPT-backend-private `{"type":"namespace",…}` tool
/// shapes that strict third-party Responses gateways reject. This covers both
/// managed xAI OAuth and API-key providers that use xAI's native Responses API.
...
"#
        ...
        let legacy_zhipu = create_provider(json!({
            "apiFormat": "openai_chat",
            "baseURL": "https://api.z.ai/api/v1"
        }));
        assert_eq!(
            resolve_codex_catalog_tool_profile(&legacy_zhipu),
            CodexCatalogToolProfile::NativeResponses
        );
    }

    #[test]
    fn native_responses_url_guard_matches_gateways_not_chat_paths() {
        for url in [
...
"#
    ...
    #[test]
    fn namespace_flatten_gate_fires_for_xai_oauth_and_api_key_responses() {
        ...
        let api_key_xai = create_provider(json!({
...
"#
        ...
        assert!(provider_needs_responses_namespace_flatten(&api_key_xai));
