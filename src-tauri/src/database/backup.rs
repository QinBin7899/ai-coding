...
const CC_SWITCH_SQL_EXPORT_HEADER: &str = "-- AI Coding SQLite 导出";
...
impl Database {
    ...
    fn validate_cc_switch_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        Err(AppError::localized(
            ...
            "仅支持导入由 AI Coding 导出的 SQL 备份文件。",
            "Only SQL backups exported by AI Coding are supported.",
        ))
    }
    ...
    {
        let db_path = get_app_config_dir().join("ai-coding.db");
        if !db_path.exists() {
            return Ok(None);
        }
    ...
    /// Validate that the external SQL created a recognizable AI Coding schema.
    ///
    ...
    fn validate_imported_schema(conn: &Connection) -> Result<(), AppError> {
        ...
        if !missing.is_empty() {
            ...
            return Err(AppError::localized(
                ...
                format!("导入的 SQL 缺少 AI Coding 必需表：{names}"),
                format!("The imported SQL is missing required AI Coding tables: {names}"),
            ));
        }
        Ok(())
    }
...
mod tests {
    ...
    impl TestHomeGuard {
        fn new() -> Self {
            ...
            let config_dir = temp_dir.path().join(".zhongguoai");
            std::fs::create_dir_all(&config_dir).expect("create isolated config directory");
            std::fs::File::create(config_dir.join("ai-coding.db"))
    ...
    fn import_rejects_cross_file_statements_and_leaves_no_file_behind() -> Result<(), AppError> {
        ...
        for (label, template) in cases {
            let target = test_home
                .path()
                .join(format!("ai-coding-authorizer-{label}.sqlite"));
    ...
    fn import_rejects_header_only_sql_and_keeps_existing_database() -> Result<(), AppError> {
        ...
        assert!(
            error.to_string().contains("required AI Coding tables")
                || error.to_string().contains("AI Coding 必需表"),
    ...
    fn import_rejects_truncated_open_transaction_and_keeps_live_database() -> Result<(), AppError> {
        ...
        let truncated = exported
            .strip_suffix("COMMIT;\nPRAGMA foreign_keys=ON;\n")
            .expect("AI Coding export should end with a committed transaction");
