...
pub async fn save_file_dialog<R: tauri::Runtime>(
    ...
    #[allow(non_snake_case)] filterName: Option<String>,
    extensions: Option<Vec<String>>,
) -> Result<Option<String>, String> {
    let dialog = app.dialog();
    let mut file_dialog = dialog.file().set_file_name(&defaultName);
    let filter_name = filterName.unwrap_or_else(|| "SQL".to_string());
    let filter_extensions = extensions.unwrap_or_else(|| vec!["sql".to_string()]);
    let filter_refs = filter_extensions.iter().map(String::as_str).collect::<Vec<_>>();
    file_dialog = file_dialog.add_filter(&filter_name, &filter_refs);
