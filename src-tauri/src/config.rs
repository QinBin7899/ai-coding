...
///   且不一定等于用户目录，可能导致 `.zhongguoai/zhongguoai.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
...
/// 获取应用配置目录路径 (~/.zhongguoai)
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    let default_dir = get_home_dir().join(".zhongguoai");

    // 兼容 v3.10.3：当用户环境存在 `HOME` 且与真实用户目录不同，
    // v3.10.3 可能在 `HOME/.zhongguoai/` 下创建/使用了数据库。
