...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.zhongguoai/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/zhongguoai.log）
            {
                    ...
                    tauri_plugin_log::Builder::default()
                        ...
                        .targets([
                            ...
                            Target::new(TargetKind::Folder {
                                path: log_dir,
                                file_name: Some("zhongguoai".into()),
                            }),
                        ])
                ...
                log::info!("=== AI Coding v{} started ===", env!("CARGO_PKG_VERSION"));
            }
            ...
            // 会话文件监听：终端里的 Claude Code / Codex 追加消息时，
            // 向前端推送 `session-manager://changed`，让会话管理页实时刷新。
            // 失败只记录日志，不影响启动。
            session_manager::watcher::start(app.handle().clone());
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::get_memory_sync_overview,
            commands::sync_memory_files,
            ...
            commands::get_session_file_stat,
            commands::search_session_contents,
