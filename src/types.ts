...
export interface Provider {
  ...
  // 新增：是否为商业合作伙伴
  isPartner?: boolean;
  // 可选：供应商元数据（仅存于 ~/.ai-coding/config.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
// 存储在本地 ~/.ai-coding/settings.json，不随数据库同步
...
/** 会话源文件的 mtime / size（兜底轮询用） */
export interface SessionFileStat {
  mtimeMs: number;
  size: number;
}

/** 后端 `session-manager://changed` 事件负载 */
...
/** 会话搜索匹配方式 */
export type SessionSearchMode = "exact" | "fuzzy";
...
/** 正文全文检索的命中结果 */
export interface SessionSearchHit extends SessionRef {
  matchCount: number;
  score: number;
  snippet: string;
}
