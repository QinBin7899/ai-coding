...
import bincodeWordmark from "@/assets/branding/bincode-wordmark.png";
...
export const ProviderIcon: React.FC<ProviderIconProps> = ({
  ...
  const titleProps = disableTitle ? {} : { title: name };

  if (icon === "bincode") {
    const numericSize =
      typeof size === "number" ? size : Number.parseFloat(String(size)) || 32;
    return (
      <img
        src={bincodeWordmark}
        alt={name}
        className={cn(
          "inline-flex flex-shrink-0 rounded-[14px] bg-black object-cover drop-shadow-[0_10px_22px_rgba(255,140,56,0.1)]",
          className,
        )}
        style={{
