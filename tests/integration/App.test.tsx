...
describe("App integration with MSW", () => {
  ...
  it("warns without blocking when removing Pi's global default provider", async () => {
    // AI Coding 默认隐藏 Pi，需要显式打开才会出现在应用切换器里。
    setSettings({ visibleApps: { ...DEFAULT_VISIBLE_APPS, pi: true } });
    localStorage.setItem("ai-coding-last-app", "pi");
    setProviders("pi", {
      custom: {
        id: "custom",
        name: "Custom Pi",
  ...
  it("hosts the Skills check-update action in the App toolbar", async () => {
    localStorage.setItem("ai-coding-last-view", "skills");
    const { default: App } = await import("@/App");
    renderApp(App);
